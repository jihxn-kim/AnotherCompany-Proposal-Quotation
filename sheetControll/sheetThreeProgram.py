...
class Sheet:
    ...
    def makeSheet(self):
        ...
        ws["A16"] = self.last_grade + "\n" + self.last_program_1 + "\n" + "프로그램 체험비"
        ws["A16"].alignment = Alignment(wrap_text=True, horizontal="center", vertical="center")

        # 차시
        ws["B14"] = self.class_num_1
        ws["B15"] = self.class_num_2
        ws["B16"] = self.class_num_3
        ...
        ws["H16"] = int(self.price_3) * int(self.last_class)
        ws["H16"].number_format = '"₩"#,##0'

        # 견적가 총합
        ws["B17"] = int(self.price_1) * int(self.first_class) + int(self.price_2) * int(self.second_class) + int(self.price_3) * int(self.last_class)
